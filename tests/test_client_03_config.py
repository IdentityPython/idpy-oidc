...
def test_json_1():
    configuration = create_from_config_file(RPConfiguration,
                                            filename=os.path.join(_dirname, 'rp_conf.json'),
                                            base_path=_dirname)
    ...
    assert set(configuration.default["services"].keys()) == {
        'discovery', 'registration', 'authorization', 'accesstoken', 'userinfo', 'end_session'}
    assert set(configuration.clients.keys()) == {'', 'bobcat', 'flop'}


def test_json_2():
    configuration = create_from_config_file(RPConfiguration,
                                            filename=os.path.join(_dirname, 'rp_conf_2.json'),
                                            base_path=_dirname)
    assert configuration
