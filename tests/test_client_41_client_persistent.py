...
class TestRPHandler(object):
    ...
    def setup(self):
        self.rp1 = RP(
            base_url=BASE_URL, client_configs=CLIENT_CONFIG, keyjar=CLI_KEY, module_dirs=["oidc"]
        )
    ...
    def test_do_provider_info(self):
        context_1 = self.rp1.get_context(GITHUB)
        issuer = self.rp1.do_provider_info(context_1)
        assert issuer == iss_id("github")

        # Make sure the service endpoints are set

        context_2 = self.rp2.get_context(GITHUB)

        _context_dump = context_1.dump()
        context_2.load(_context_dump)
        _service_dump = context_1.get_services().dump()
        context_2.get_services().load(
            _service_dump, init_args={"upstream_get": context_2.upstream_get}
        )

        for service_type in ["authorization", "accesstoken", "userinfo"]:
            _srv = context_2.get_service(service_type)
            _endp = context_2.provider_info[_srv.endpoint_name]
            assert _srv.endpoint == _endp

    def test_do_client_registration(self):
        context_1 = self.rp1.get_context(GITHUB)
        self.rp1.do_client_registration(context_1, issuer=GITHUB)

        # only 2 things should have happened

        assert self.rp1
        assert not context_1.get_usage("post_logout_redirect_uris")
