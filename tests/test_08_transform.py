...
from idpyoidc.client.metadata.oidc import Metadata as OIDCMetadata
...
class TestTransform:
    ...
    def setup(self):
        supported = OIDCMetadata._supports.copy()
        for service in [
    ...
    def test_oidc_setup(self):
        ...
        metadata.supported_to_preferred(supported=self.supported, base_url='https://example.com')
        ...
        assert set(metadata.prefer.keys()) == {'application_type',
                                               'default_max_age',
                                               'encrypt_request_object_supported',
                                               'encrypt_userinfo_supported',
                                               'grant_types_supported',
        ...
        reg_claim = []
        for key, spec in OIDCMetadata.registration_request.c_param.items():
            _pref_key = OIDCMetadata.register2preferred.get(key, key)
        ...
        l_to_s = []
        non_oidc = []
        for key, pref_key in OIDCMetadata.register2preferred.items():
    ...
    def test_provider_info(self):
        ...
        metadata = OIDCMetadata()
        metadata.supported_to_preferred(supported=self.supported,
        ...
        assert metadata.get_preference('scopes_supported') == ['openid']
        assert metadata.get_preference("response_modes_supported") == ['query', 'form_post']
        # The OP supports less than the RP
        assert metadata.get_preference("response_types_supported") == ['code', 'id_token',
                                                                       'code id_token']
...
class TestTransform2:
    ...
    def setup(self):
        ...
        supported = self.metadata._supports.copy()
        for service in [
            'idpyoidc.client.oidc.access_token.AccessToken',
            'idpyoidc.client.oidc.authorization.Authorization',
            'idpyoidc.client.oidc.backchannel_authentication.BackChannelAuthentication',
        ...
        self.metadata.load_conf(preference, self.supported)
    ...
    def test_registration_response(self):
        ...
        self.metadata.supported_to_preferred(supported=self.supported,
                                             base_url='https://example.com',
                                             info=provider_info_response)

        registration_request = self.metadata.create_registration_request(self.supported)
        ...
        to_use = self.metadata.preferred_to_registered(supported=self.supported,
