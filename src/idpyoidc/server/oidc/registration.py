...
class Registration(Endpoint):
    ...
    def match_claim(self, claim, val):
        ...
        # Use my defaults
        _my_key = _context.metadata.register2preferred.get(claim, claim)
        try:
        ...
        try:
            _claim_spec = _context.metadata.registration_response.c_param[claim]
    ...
    def filter_client_request(self, request: dict) -> dict:
        ...
        for key, val in request.items():
            if key not in _context.metadata.register2preferred:
                _args[key] = val
                continue
        ...
        for item in ["id_token_signed_response_alg", "userinfo_signed_response_alg"]:
            if item in request:
                if request[item] in _context.provider_info[
                        _context.metadata.register2preferred[item]]:
