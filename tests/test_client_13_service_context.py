...
def test_client_info_init():
    ...
    entity = Entity(entity_id=BASE_URL, config=config)
    entity_copy = Entity().load(entity.dump())

    srvcnx = entity_copy.get_context()
...
def test_client_filename():
    ...
    fname = entity.get_context().filename_from_webname("https://example.com/rq12345")
...
class TestClientInfo(object):
    ...
    def test_verify_alg_support(self):
        ...
        assert verify_alg_support(self.service_context, "RS256", "id_token", "signing_alg")
        assert verify_alg_support(self.service_context, "RS512", "id_token", "signing_alg") is False

        assert verify_alg_support(self.service_context, "RSA1_5", "userinfo", "encryption_alg")

        # token_endpoint_auth_signing_alg_values_supported
        assert verify_alg_support(
            self.service_context, "ES256", "token_endpoint_auth", "signing_alg"
        )
