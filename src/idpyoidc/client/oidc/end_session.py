...
class EndSession(Service):
    ...
    def get_id_token_hint(self, context, request_args=None, **kwargs):
        """
        ...
        _id_token = context.cstate.get_claim(kwargs["state"], claim="id_token")
        if _id_token:
            request_args["id_token_hint"] = _id_token

        return request_args, {}

    def add_post_logout_redirect_uri(self, context, request_args=None, **kwargs):
        if "post_logout_redirect_uri" not in request_args:
            _uri = context.get_usage("post_logout_redirect_uris")
            if _uri:
