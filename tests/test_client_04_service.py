...
class TestService:
    ...
    def create_service(self):
        ...
        self.service = self.entity.get_service(self.service_context, "authorization")
        self.service_context.map_supported_to_preferred()

        # The opponent
        self.issuer = "https://op.example.com/"
        self.issuer_keyjar = init_key_jar(key_defs=KEYDEFS, issuer_id=self.issuer)
        # Let the client have the opponents public keys
        self.service_context.keyjar.import_jwks(self.issuer_keyjar.export_jwks(issuer_id=self.issuer),
    ...
    def test_gather_request_args(self):
        ...
        args = self.service.gather_request_args(self.service_context, issuer='', state="state")
        assert args == {
            "response_type": "code",
            "state": "state",
            "client_id": "CLIENT",
    ...
    def test_parse_response_urlencoded(self):
        ...
        arg = self.service.parse_response(self.service_context, resp1)
    ...
    def test_parse_response_json(self):
        ...
        # _sign_key = self.issuer_keyjar.get_signing_key()
    ...
    def test_parse_response_jwt(self):
        ...
        self.service.response_cls = AuthorizationResponse
        self.service_context.issuer = self.issuer
        self.service_context.client_id = "client"

        _sign_key = self.issuer_keyjar.get_signing_key(issuer=self.issuer)
        resp1 = AuthorizationResponse(code="auth_grant", state="state", iss=self.issuer).to_jwt(
            key=_sign_key, algorithm="RS256"
        )
        arg = self.service.parse_response(self.service_context, resp1)
        assert isinstance(arg, AuthorizationResponse)
        assert arg.to_dict() == {'code': 'auth_grant', 'iss': 'https://op.example.com/', 'state': 'state'}
...
class TestAuthorization(object):
    ...
    def create_service(self):
        ...
        self.service = self.entity.get_service(self.service_context, "")

    def test_construct(self):
        req_args = {"foo": "bar"}
        _req = self.service.construct(self.service_context, request_args=req_args, state="state")
    ...
    def test_get_request_parameters(self):
        ...
        _info = self.service.get_request_parameters(self.service_context, request_args=req_args, state="state")
    ...
    def test_request_init(self):
        ...
        self.service.endpoint = "https://example.com/authorize"
        _info = self.service.get_request_parameters(self.service_context, request_args=req_args)
        assert set(_info.keys()) == {"url", "method", "request"}
        msg = Message().from_urlencoded(self.service.get_urlinfo(_info["url"]))
    ...
    def test_response(self):
        ...
        assert set(_info.keys()) == {"url", "method", "request"}
        msg = Message().from_urlencoded(self.service.get_urlinfo(_info["url"]))
        self.service_context.cstate.set(_state, msg)

        resp1 = AuthorizationResponse(code="auth_grant", state=_state)
        response = self.service.parse_response(self.service_context, resp1.to_urlencoded(), "urlencoded", state=_state)
        self.service.update_service_context(self.service_context, response, key=_state)
        assert self.service_context.cstate.get(_state)
