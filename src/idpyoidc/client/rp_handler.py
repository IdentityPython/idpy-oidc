...
class RPHandler(object):
    ...
    def _get_response_type(self, context, req_args: Optional[dict] = None):
        if req_args:
            return req_args.get("response_type",
                                context.metadata.get_usage("response_types")[0])
        else:
            return context.metadata.get_usage("response_types")[0]
    ...
    def init_authorization(
        ...
        _scope = _context.metadata.get_usage("scopes_supported")
        if _scope:
    ...
    def get_response_type(client):
        ...
        """
        return client.service_context.metadata.get_usage("response_types")[0]
